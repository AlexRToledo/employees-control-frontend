...
class UsersComponent extends IComponent{
    constructor() {
        ...
        this.state = {
            ...
            keys: [
                ...
                {field: 'status', label: 'Estado', 'options': {true: 'Activado', false: 'Deshabilitado'}}
            ],
            title: [
                {field: 'username', label: 'Nombre y Apellidos'},
            ],
            params: null
        };
    ...
    async changePageLoadData(page=1) {
        try {
            const res = await service.GetList(page, this.state.params);
            if(res && !res.error) {
                this.setState({
                    data: res.data
                });
                ...
                <div className={'content-wrapper'}>
                    ...
                    <div className={"uk-grid"} >
                        ...
                        <div className={'uk-width-1-5@m side-actions'}>
                            <AddButton title={'Nuevo'} link={'/dashboard/users/create'}/>
                            <hr/>
